...
import com.test.gmall.ums.vo.UmsAdminLoginParam;
import com.test.gmall.ums.vo.UmsAdminParam;
import com.test.gmall.admin.utils.JwtTokenUtil;
...
/**
 * 后台用户管理
 * springMVC支持 JSR303 方式进行校验
 * 1、springboot默认导第三方的校验框架 hibernate-validator
 * 使用JSR303的三大步
 * 1）、在需要验证的JavaBean属性上标校验注解
 * 2）、在action方法对应实体类参数前加 @Validated
 *      当自动校验出错时，不执行该方法
 * 3）、手动处理校验出错，加 BindingResult result
 */
@Slf4j
@CrossOrigin   //浏览器报错:Error: Incorrect contents fetched, please reload. 加上该注解
@RestController
...
public class UmsAdminController {
    ...
    public Object register(@Validated @RequestBody UmsAdminParam umsAdminParam, BindingResult result) {
        Admin admin = null;
        //TODO 完成注册功能
//        //获取校验出错数量
